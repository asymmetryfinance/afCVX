...
library Zap {
    ...
    function swapCvxToClevCvx(uint256 amountIn, uint256 minAmountOut) external returns (uint256) {
        CVX.safeApprove(CVX_CLEVCVX_POOL, amountIn);
        return ICurveFactoryPlainPool(CVX_CLEVCVX_POOL).exchange(0, 1, amountIn, minAmountOut);
    }

    function swapCvxCrvToCvx(uint256 amountIn, uint256 minAmountOut) external returns (uint256) {
        // cvxCRV -> CRV
        CVXCRV.safeApprove(CVXCRV_CRV_POOL, amountIn);
        uint256 crvAmount = ICurveFactoryPlainPool(CVXCRV_CRV_POOL).exchange(1, 0, amountIn, 0);
